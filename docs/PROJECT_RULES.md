...
At least **five canonical test vectors** must be generated and frozen:

1. **Vector 1:** Minimum packet (empty payload) - boundary condition
2. **Vector 2:** Typical command (e.g., relay set) - common operation
3. **Vector 3:** Maximum payload packet - boundary condition
4. **Vector 4:** Random/complex payload - general correctness
5. **Vector 5:** CRC verification failure case - error detection

Each vector must:
- Be generated by executing firmware reference implementation
- Have transmitted packet bytes captured from actual USB/serial
- Be stored in `shared/docs/protocol/test_vectors/vector_NNN.json`
- Be passed by all independent implementations
...
This requirement ensures protocol correctness is verified at the system integration level.
